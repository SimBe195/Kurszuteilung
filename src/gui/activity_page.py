...
from activity import WEEKDAYS, Activity, Timespan
...
class ActivityPage(ctk.CTkFrame):
    def __init__(self, master: Any) -> None:
        ...
        for idx, title in enumerate(
            ...
            column_title = ctk.CTkLabel(
                self.activity_view, text=title, font=ctk.CTkFont(size=20)
            )
    ...
    def remove_activity(self):
        if (activity := search_activity(self)) is not None and confirm_choice(
            self, f'Kurs "{activity.name}" wirklich entfernen?'
        ):
        ...
        if (activity := search_activity(self)) is not None:
            ...
            for student in State().students:
                if activity.id in student.preferences:
                    student.preferences[activity_2.id] = student.preferences[
                        activity.id
                    ]
    ...
    def display_activities(self):
        ...
        for row, activity in enumerate(state.activities, start=1):
            id_label = ctk.CTkLabel(
                self.activity_view, text=str(activity.id), font=ctk.CTkFont(size=16)
            )
            id_label.grid(row=row, column=0, pady=5, sticky="nsew")

            name_label = ctk.CTkLabel(
                self.activity_view, text=activity.name, font=ctk.CTkFont(size=16)
            )
            name_label.grid(row=row, column=1, pady=5, sticky="nsew")

            min_capacity_label = ctk.CTkLabel(
                self.activity_view,
                text=str(activity.min_capacity),
            ...
            max_capacity_label = ctk.CTkLabel(
                self.activity_view,
                text=str(activity.max_capacity),
                font=ctk.CTkFont(size=16),
            )
            max_capacity_label.grid(row=row, column=3, pady=5, sticky="nsew")

            timespan_label = ctk.CTkLabel(
                self.activity_view,
                text=str(activity.timespan),
                font=ctk.CTkFont(size=16),
            )
            ...
            first_date_label = ctk.CTkLabel(
                self.activity_view,
                text=str(activity.first_date),
                font=ctk.CTkFont(size=16),
            )
            first_date_label.grid(row=row, column=5, pady=5, sticky="nsew")

            grades_text = ", ".join(
                [str(grade) for grade in range(1, 5) if activity.is_valid_grade(grade)]
            )
            grades_label = ctk.CTkLabel(
                self.activity_view, text=grades_text, font=ctk.CTkFont(size=16)
            )
...
class ModifyActivityDialog(ctk.CTkToplevel):
    def __init__(self, master: Any, title: str) -> None:
        ...
        current_row += 1
        name_label = ctk.CTkLabel(
            name_frame, text="Bezeichnung:", font=ctk.CTkFont(size=16)
        )
        ...
        current_row += 1
        timespan_label = ctk.CTkLabel(
            timespan_frame, text="Zeitslot:", font=ctk.CTkFont(size=16)
        )
        ...
        self.timespan_from_hour_option = ctk.CTkOptionMenu(
            timespan_frame,
            width=50,
            values=[f"{h:02d}:{15 * m:02d}" for h in range(8, 18) for m in range(4)],
        )
        self.timespan_from_hour_option.grid(row=0, column=2, padx=10)
        ctk.CTkLabel(timespan_frame, text="bis", font=ctk.CTkFont(size=16)).grid(
            row=0, column=3
        )
        ...
        current_row += 1
        first_date_label = ctk.CTkLabel(
            first_date_frame, text="Erster Termin:", font=ctk.CTkFont(size=16)
        )
        ...
        capacity_label = ctk.CTkLabel(
            capacity_frame, text="Teilnehmerzahl:", font=ctk.CTkFont(size=16)
        )
        capacity_label.grid(row=0, column=0, columnspan=2, padx=20, pady=20, sticky="w")
        self.min_capacity_label = ctk.CTkLabel(
            capacity_frame, font=ctk.CTkFont(size=14)
        )
        ...
        self.max_capacity_label = ctk.CTkLabel(
            capacity_frame, font=ctk.CTkFont(size=14)
        )
        ...
        current_row += 1
        valid_grades_label = ctk.CTkLabel(
            valid_grades_frame, text="Klassen:", font=ctk.CTkFont(size=16)
        )
        valid_grades_label.grid(
            row=0, column=0, columnspan=4, padx=20, pady=20, sticky="w"
        )
    ...
    def update_capacity_labels(self):
        self.min_capacity_label.configure(
            text=f"Min.\t{self.min_capacity_var.get():>2}"
        )
        self.max_capacity_label.configure(
            text=f"Max.\t{self.max_capacity_var.get():>2}"
        )
    ...
    def insert_activity_data(self, activity: Activity):
        ...
        for check_var, valid in zip(
            self.valid_grades_check_vars, activity.valid_grades
        ):
            check_var.set(valid)
        self.update_capacity_labels()
    ...
    def on_accept(self):
        ...
        from_hour, from_minute = map(
            int, self.timespan_from_hour_option.get().split(":")
        )
        ...
        if min_capacity > max_capacity:
            open_error_popup(
                self, "Min. Teilnehmerzahl muss kleiner sein als max. Teilnehmerzahl!"
            )
            return

        try:
            timespan = Timespan.from_day_hour_minute(
                day, from_hour, from_minute, day, to_hour, to_minute
            )
